...
# The purpose of this project is to determine: 1. The total number of votes cast. 
...
csv_path = os.path.join("PyPoll\Resources", "election_data.csv") # Reading the CSV file

output_data = os.path.join('PyPoll\Analysis', 'Report.txt')      #Writing on the analysis file (exporting file to txt)
...
with open(csv_path) as election_data:
    ...
    #comparing two lists
        ...
        # Assigning a  key (candidate_Names) and value to the dictionary 
         ...
         # A dictionary: stores the names (keys) of the three candidates and their total vote each won (value)

        Candidate_Vote[candidate_Name] = Candidate_Vote[candidate_Name] +1  
   

    Total_Count = Total_Votes    # Added here for the purpose of printing only 
